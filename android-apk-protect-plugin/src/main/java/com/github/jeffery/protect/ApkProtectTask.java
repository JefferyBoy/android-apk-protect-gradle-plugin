 ...
 * @date 2022/9/21
 */
public class ApkProtectTask extends DefaultTask {

    private String apkPath;
    private ApkProtectExtension config;
    private SigningConfig signConfig;
    private final Logger log = Logging.getLogger(getClass());

    @Inject
    public ApkProtectTask(String apkPath, ApkProtectExtension config, SigningConfig signConfig) {
    ...
    public void execute() {
        ...
        try {
            log.quiet("Prepare protect apk {}", apkFile.getAbsolutePath());
...
//                        File(signedApk).renameTo(apkFileOut)
     ...
     * 检查配置是否正确
     */
    private boolean checkRequireConfiguration(ApkProtectExtension model) {
    ...
    private String signApk(String filePath, SigningConfig signConfig) {
        ...
        if (signConfig.getStoreFile() == null || !signConfig.getStoreFile().exists()) {
